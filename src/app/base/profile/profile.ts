...
import { debounce, email, form, maxLength, minLength, pattern, required, validateHttp } from '@angular/forms/signals';
...
export class Profile {
  ...
  userDataForm = form(this.userData, (schemaPath) => {
    ...
    minLength(schemaPath.first_name, 2, { message: 'First name must be at least 2 characters long.' });
    maxLength(schemaPath.first_name, 20, { message: 'First name must not exceed 20 characters.' });
    minLength(schemaPath.last_name, 2, { message: 'Last name must be at least 2 characters long.' });
    ...
    pattern(schemaPath.first_name, /^[A-Za-z'-]+$/, { message: 'First name can only contain letters, apostrophes, and hyphens.'});
    pattern(schemaPath.last_name, /^[A-Za-z'-]+$/, { message: 'Last name can only contain letters, apostrophes, and hyphens.'});
