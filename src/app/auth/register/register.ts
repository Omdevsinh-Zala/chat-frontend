...
import { debounce, email, Field, form, maxLength, minLength, pattern, required, validateHttp } from '@angular/forms/signals';
import { MatIcon } from '@angular/material/icon';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';
import { MatTooltipModule } from '@angular/material/tooltip';
import { GlobalResponse, RegisterModel } from '../../models/auth.model';
...
export class Register {
  ...
  registerForm = form(this.registerModel, (schemaPath) => {
    ...
    pattern(schemaPath.firstName, /^[A-Za-z'-]+$/, { message: 'First name can only contain letters, apostrophes, and hyphens.'});
    pattern(schemaPath.lastName, /^[A-Za-z'-]+$/, { message: 'Last name can only contain letters, apostrophes, and hyphens.'});

    pattern(schemaPath.username, /^\S*$/, { message: 'Username must not contain spaces.'});
    pattern(schemaPath.username, /^[a-zA-Z0-9_]+$/, { message: 'Username can only contain letters, numbers, and underscores.'});

    pattern(schemaPath.password, /^\S*$/, { message: 'Password must not contain spaces.' });
    pattern(schemaPath.password, /[A-Z]/, { message: 'Password must contain at least one uppercase letter.' });
    pattern(schemaPath.password, /[a-z]/, { message: 'Password must contain at least one lowercase letter.' });
    pattern(schemaPath.password, /[0-9]/, { message: 'Password must contain at least one number.' });
    pattern(schemaPath.password, /[\W_]/, { message: 'Password must contain at least one special character.' });
